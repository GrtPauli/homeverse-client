...
export const ChatRoom: FC<IProps> = ({ db, currentUser, recepientId }) => {
  const dummy = useRef();
  ...
  // ))
  const filter = query(messagesRef, where("roomIds", 'array-contains-any', [uid, recepientId]))
  ...
  return (<>
      <div className="h-full w-full relative bg-light-white">
          ...
          <div className='flex flex-col gap-5 px-5 pt-5 bg-light-white overflow-scroll'>
            {messages && messages.map((msg, i) => <ChatMessage key={i} currentUserId={uid} message={msg} />)}
            <span ref={dummy}></span>
          </div>
          ...
          <div className="absolute bg-light-white py-4 px-5 w-full bottom-0 border-t">
            <form className='flex gap-3 items-center' onSubmit={sendMessage}>
              ...
              <button className='bg-primary text-white cursor-pointer' type="submit" disabled={!formValue}>🕊️</button>
            </form>
          </div>
      </div>
  </>)
...
const ChatMessage: FC<ChatMessageProps> = ({message, currentUserId}) => {
  ...
  return (<>
      <div className={`flex gap-3 ${from === currentUserId && 'self-end'}`}>
        <div 
            className={
                `py-1.5 px-3 gap-3 rounded-b-lg flex justify-between items-end
                ${from === currentUserId ? 'bg-primary rounded-tl-lg text-light-white' 
