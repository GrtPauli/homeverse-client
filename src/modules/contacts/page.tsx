      ...
      render: (value) => (
        <div className='px-3'>
          <Formik
            ...
            initialValues={{}}
          >
            <AppSelectInput className='text-[13px] border border-colors-opal/40 w-full rounded py-2 outline-none px-3' options={Object.values(ContactStatus)} name='status'/>
          </Formik>
        </div>
      ),
    },
...
export const ContactsPage = () => {
    ...
    const {loading, getContactInfo, contacts, contactRequests, acceptContactRequest} = useContactContext()
    ...
    <AgentHubLayout>
        <div className='w-full flex justify-center'>
            <div className='w-full bg-light-white rounded shadow-lg px-10 pt-8 pb-2'>
                ...
                {!loading && <Table loading={loading} className='translate-y-2' bordered columns={columns} dataSource={contacts}/>}
                <button onClick={() => setOpen(true)} className='text-primary text-sm font-medium -translate-y-8'>
          ...
          >
            <>
              ...
              <div className='flex flex-col gap-8'>
                {contactRequests.length > 0 ? contactRequests.map((item, i) => (
                  <div key={i} className='flex justify-between items-center'>
                    ...
                    <div className='flex items-center gap-5'>
                      <button onClick={() => handleAccept(item)} className='bg-green-500 text-light-white rounded-full flex items-center justify-center w-6 h-6'>
                        <CheckOutlined/>
                      </button>
