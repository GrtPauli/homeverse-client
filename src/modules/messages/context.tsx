import { Dispatch, FC, ReactNode, SetStateAction, createContext, useContext, useRef, useState } from "react";
...
const MessageContextProvider: FC<IProps> = ({ children }) => {
    ...
    const fetchContactInfos = (usernames: string[]) => {
        agoraConn.fetchUserInfoById(["Jake123"])
        .then((res) => {
            console.log(res)
        }).catch(err => {
            console.log(err)
    ...
    const getConversationList = () => {
        agoraConn.getConversationlist({pageNum: 1, pageSize: 20})
        .then((res) => {
    ...
    const getHistoryMessages = (targetId: string) => {
        ...
        agoraConn.getHistoryMessages(options).then((res)=>{
